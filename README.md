...
per finding). It runs with **whichever judge you selected**, using the strongest
active one (`--claude-judge` > `--codex-judge` > `--ollama-judge`) so a weak
verifier can't drop a strong judge's legitimate finding. Findings that survive
show the verifier's reason.
